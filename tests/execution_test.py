...
async def test_durable_execution_client_selection_env_normal_result():
    ...
    with (
        patch(
            "async_durable_execution.core.execution.ThreadedSyncLambdaClient"
        ) as mock_lambda_client,
        patch(
            "async_durable_execution.core.execution.create_default_client",
...
async def test_durable_execution_reuses_default_async_client_on_warm_invocations():
    ...
    with (
        patch(
            "async_durable_execution.core.execution.AsyncLambdaClient",
...
async def test_durable_execution_logs_checkpoint_error_extras_from_background_thread():
    ...
    with patch("async_durable_execution.core.execution.logger", mock_logger):
        response = await run_handler(
            test_handler, invocation_input, lambda_context, service_client=mock_client
        )
        assert response["Status"] == InvocationStatus.FAILED.value
