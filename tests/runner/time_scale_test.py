...
def test_get_time_scale_reads_valid_float(monkeypatch):
    monkeypatch.setenv("DURABLE_EXECUTION_TIME_SCALE", "0.25")

    assert get_time_scale() == 0.25


def test_get_time_scale_ignores_invalid_value(monkeypatch, caplog):
...
def test_get_time_scale_ignores_negative_value(monkeypatch, caplog):
...
def test_scale_delay_without_minimum(monkeypatch):
    monkeypatch.setenv("DURABLE_EXECUTION_TIME_SCALE", "0.5")

    assert scale_delay(10, minimum=0) == 5.0


def test_scale_delay_respects_minimum_without_exceeding_original(monkeypatch):
