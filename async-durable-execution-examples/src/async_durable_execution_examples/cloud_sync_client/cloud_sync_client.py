...
from async_durable_execution.client import create_default_client


_sync_lambda_client = create_default_client()
...
async def handler(event: dict[str, Any]) -> dict[str, str]:
    """Run on Lambda with the sync botocore client even when aioboto is installed."""
