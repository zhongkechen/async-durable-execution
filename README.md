...
- **[Async-native durable code](https://zhongkechen.github.io/async-durable-execution/official-python-sdk-comparison.html#programming-model)** - Durable handlers, `flow` nodes, `@durable_callable` functions, and serializers use `async def`; synchronous steps use `@durable_step`. Child contexts, callback submitters, map item functions, parallel branches, and condition checks may be async or sync. Synchronous leaf callables run in a worker thread; functions that compose durable operations use `async def`.
