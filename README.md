...
- **[Async-native durable code](https://zhongkechen.github.io/async-durable-execution/official-python-sdk-comparison.html#programming-model)** - Durable handlers, `flow` nodes, and `@durable_callable` functions use `async def`; synchronous steps use `@durable_step`. Child contexts, callback submitters, map item functions, parallel branches, condition checks, and serializers may be async or sync. Synchronous leaf callables and `SyncSerDes` implementations run in a worker thread; functions that compose durable operations use `async def`.
...
- `sync_functions/` for synchronous durable functions and methods, map items, and parallel branches
