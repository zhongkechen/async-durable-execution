...
> **V3 development branch.** This is a new implementation with the v2 top-level public API.
> See the [rewrite and validation status](docs/v3-rewrite.md) for completed checks
...
**Build long-running AWS Lambda workflows with native
...
**V3 preserves the v2 top-level public API.** The replacement journal runtime is checked
against the captured public API contract, newly authored behavior tests, and
application examples. Private imports and v2 execution histories are not preserved.

The [AWS Durable Execution conformance suite](https://github.com/aws/aws-durable-execution-conformance-tests)
remains a release check against deployed Lambda functions. This development
branch has not yet completed that cloud validation.
...
- **[Suspension-aware cleanup and compensation](https://zhongkechen.github.io/async-durable-execution/terminal-scopes.html)** - Register durable cleanup and reverse-order compensation that runs at logical completion or failure without firing when a wait, callback, retry, or branch merely suspends.
- **[Background operation tasks](https://zhongkechen.github.io/async-durable-execution/advanced-usage.html#background-operation-tasks)** - Durable operations such as `step(...)`, `wait(...)`, `invoke(...)`, `recurse(...)`, `run_in_child_context(...)`, `terminal_scope(...)`, and `flow(...)` return `asyncio.Task` objects, so independent operations can run in the background and be awaited together with `asyncio.gather` without using `parallel()` or `map()`.
...
- `terminal_scope/` for callback-safe cleanup and failure compensation
...
- **[Durable Terminal Scopes](https://zhongkechen.github.io/async-durable-execution/terminal-scopes.html)** - Run cleanup and compensation at logical terminal outcomes without treating suspension as failure
