...
def _step_envelope(result: Any, name: str) -> dict[str, Any]:
    ...
    assert envelope[_FILESYSTEM_MARKER] == 3
    return envelope
...
async def test_filesystem_serdes_always_round_trips_across_replay(
    ...
    if not _is_cloud(request):
        ...
        assert order_digest == order_envelope["sha256"]
        assert len(order_payload) == order_envelope["bytes"]
...
async def test_filesystem_serdes_overflow_replays_inline_and_file_payloads(
    ...
    if not _is_cloud(request):
        ...
        assert hashlib.sha256(payload).hexdigest() == file_envelope["sha256"]
        assert len(payload) == file_envelope["bytes"]
