...
from examples.invoke import invoke, price_order_child


CHILD_HANDLER = "examples.invoke.price_order_child.handler"


async def test_price_order_child(durable_runner) -> None:
    async with durable_runner(
        handler=price_order_child.handler, input={"order_id": "order-123"}, timeout=30
    ) as runner:
        result = await runner.run()

    assert result.status is InvocationStatus.SUCCEEDED, result.error
    assert result.get_deserialized_result() == {"price": 42, "currency": "USD"}
