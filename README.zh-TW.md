...
這個分支特別著重於讓非同步 Python 能自然地搭配持久函式使用。公開 API 在持久操作邊界仍維持同步介面，但使用者提供的持久可呼叫物件現在必須對處理常式、步驟、子情境、回呼提交器與條件檢查使用 `async def`。

## 專案緣由

建立這個專案首先是因為官方 Python SDK 不支援 `async`/`await`，導致它很難與其他 `asyncio` 函式庫良好協作。其次，官方 Python SDK 也缺少其他官方 SDK 已支援的能力，例如 operation 的背景執行和平行執行。

這個 SDK 解決了這些問題，並為在 Python 中建構非同步優先的持久工作流程帶來諸多強化。它也採用更 Pythonic 的風格，讓 API 在現代 Python 程式碼中用起來更自然。
