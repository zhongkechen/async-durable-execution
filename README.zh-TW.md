...
- **[不依賴模型的 Lambda 用戶端](https://zhongkechen.github.io/async-durable-execution/advanced-usage.html#lambda-client-selection)** - SDK 自主管理 Lambda REST 線路格式，不依賴 botocore 服務模型。安裝選用的 `aioboto` extra 後使用 HTTPX 傳送請求；否則相同請求會透過非同步配接器使用 botocore 的同步 HTTP 傳輸。
...
`aioboto` extra 會安裝 HTTPX，SDK 使用它進行非同步且不依賴模型的
