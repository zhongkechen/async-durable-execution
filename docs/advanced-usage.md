...
Operation identities are allocated synchronously before tasks are scheduled.
The journal supervises task lifetime on every supported Python version, using
standard asyncio tasks.
...
V3 uses the published botocore Lambda service model for request serialization,
endpoint selection, and signing. The `httpx` extra selects an asynchronous HTTPX
transport; without it, blocking botocore calls run outside the event loop. The
`aioboto` extra remains an installation alias.

Explicit synchronous and asynchronous API clients remain supported through the
`boto3_client` argument. `create_default_sync_client()` returns a standard
botocore Lambda client. A `service_client` may supply the public durable service
interface instead.

The replacement has a new persisted value format and operation namespace. Finish
existing executions on their original SDK version before switching handlers;
private module paths and prior execution histories are not migration interfaces.
