...
The public reservation interface shares the journal interpreter used by SDK
operations. A reservation allocates an opaque ticket in its owning scope; selecting
a primitive determines the effect interpreted by the journal. Private module
paths are not supported imports.
