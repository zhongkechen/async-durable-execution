...
async def test_replay_safe_helpers_use_default_step_names(monkeypatch):
    ...
    with (
        patch(
            "async_durable_execution.replay_safe._random.random",
            return_value=0.25,
        ),
        patch(
            "async_durable_execution.replay_safe.datetime",
        ) as mock_datetime,
        patch(
            "async_durable_execution.replay_safe._timestamp_value",
            return_value=1783821723.5,
        ),
        patch(
            "async_durable_execution.replay_safe._uuid.uuid4",
